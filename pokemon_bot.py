...
def run_flask():
    # Hämtar porten som Render kräver via miljövariabeln PORT
...
def kolla_webhallen_pokemon():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[{time.strftime('%H:%M:%S')}] Felkod: {response.status_code} (Ev. blockering)")
            return

        data = response.json()
        produkter = data.get("products", [])
        print(f"[{time.strftime('%H:%M:%S')}] SUCCÉ! Analyserar {len(produkter)} produkter.")
        ...
        for prod in produkter:
            ...
            bild_url = ""
            if isinstance(bilder, list) and len(bilder) > 0:
                bild_url = bilder[0].get("large", "") if isinstance(bilder[0], dict) else ""
...
if __name__ == "__main__":
    ...
    # 1. Starta den fejkade webbservern i en egen tråd så Render blir nöjd
    t = Thread(target=run_flask)
    t.daemon = True  # Gör att tråden stängs om huvudprogrammet dör
    t.start()
    
    # 2. Starta din vanliga Pokémon-övervakningsloop
    while True:
